...
def GetList(vect):
    list_vect = grass.read_command("v.db.select", map=vect, columns="cat",
                                   flags="c", quiet=True)
...
def MakeGrid(n, w, s, e, nsres, ewres, out):
    ...
    grass.run_command("g.region", n=n, s=n - nsres * rows, e=w + ewres * cols,
                      w=w, quiet=True)
    grass.run_command("v.mkgrid", map=out, quiet=True,
                      grid="{ro},{co}".format(ro=rows, co=cols))
...
def GetRefBox(ref, ref_box, k_box, processid):
    ...
    grass.run_command("g.region", n=grass.region()['n'] + ns_ext / 2,
                      s=grass.region()['s'] - ns_ext / 2,
                      w=grass.region()['w'] - ew_ext / 2,
                      e=grass.region()['e'] + ew_ext / 2, quiet=True)
    grass.run_command("v.in.region", quiet=True,
                      output="new_box_{pr}".format(pr=processid))
    grass.run_command("v.overlay", ainput=ref, atype="line", operator="and",
                      binput="new_box_{pr}".format(pr=processid), btype="area",
                      output=ref_box, quiet=True)
    grass.run_command("g.remove", type="vect", flags="f", quiet=True,
                      name="new_box_{pr}".format(pr=processid))
    grass.run_command("g.region", n=N, s=S, e=E, w=W, quiet=True)


def AddCol(vect, t):
    list_c = []
    list_col = ((grass.read_command("db.describe", table=vect, flags="c",
                                    quiet=True)).split("\n"))[2:-1]
    for c in list_col:
        list_c.append((c.split(":")[1]).lstrip())
    if t not in list_c:
        grass.run_command("v.db.addcolumn", map=vect, quiet=True,
                          columns="{na} double".format(na=t))
...
def main():
    ...
    if (len(tol_eval) > 0 and len(str(tol_max)) > 0):
        grass.fatal("Please specify only one between <tol_eval> or <tol_max> "
                    "parameters")

    if (len(tol_eval) == 0 and len(str(tol_max)) == 0):
        grass.fatal("Please specify almost one between <tol_eval> or <tol_max>"
                    " parameters")

    # Check grid parameters
    if (len(grid) > 0 and (len(ul_grid) > 0 or len(lr_grid) > 0 or
       len(box_grid) > 0 or len(output) > 0)):
        grass.warning("A <grid> vector has been specified. All the others "
                      "parameters will be ignored")

    if len(grid) == 0:
        if (len(ul_grid) > 0 or len(lr_grid) > 0 or len(box_grid) > 0) and \
           (len(ul_grid) == 0 or len(lr_grid) == 0 or len(box_grid) == 0 or
           len(output) == 0):
            grass.fatal("Please specify all the required parameters for grid "
                        "generation: <ul_grid>,<lr_grid>,<box_grid> and "
                        "<output>.")

    if (len(grid) == 0 and len(ul_grid) == 0 and len(lr_grid) == 0 and
       len(box_grid) == 0 and len(output) == 0):
        grass.fatal("No grid specified. The accuracy will be calculated on the"
                    " whole current region. Please specify the name for the "
                    "grid output vector map")
    ...
    if len(tol_eval) > 0:
        ...
        for item in list_tol:
            AddCol(output, "t_{it}".format(it=item))
            AddCol(output, "p_{it}".format(it=item))

        for k in list_box:
            grass.run_command("v.extract", input=output, output=k_box,
                              where="cat={st}".format(st=k), quiet=True)
            grass.run_command("v.overlay", ainput=osm, atype="line",
                              binput=k_box, btype="area", operator="and",
                              output=osm_box, quiet=True)
            l_osm = length(osm_box)
            grass.run_command("v.db.update", map=output, column="OSM",
                              value=l_osm, where="cat={st}".format(st=k),
                              quiet=True)
            grass.run_command("v.overlay", ainput=ref, atype="line",
                              binput=k_box, btype="area", operator="and",
                              output=ref_box, quiet=True)
            feat_ref_box = int(((grass.read_command("v.info", map=ref_box,
                                                    flags="t")).split("\n")[2]).split("=")[1])
            if feat_ref_box > 0:
                for item in list_tol:
                    val = CalcTol(ref_box, osm_box, float(item))
                    grass.run_command("v.db.update", map=output, value=val,
                                      column="t_{it}".format(it=item),
                                      where="cat={st}".format(st=k),
                                      quiet=True)
                    grass.run_command("v.db.update", map=output,
                                      column="p_{it}".format(it=item),
                                      value=val * 100.0 / l_osm, quiet=True,
                                      where="cat={st}".format(st=k))
            grass.run_command("g.remove", type="vect", pattern=processid,
                              flags="fr", quiet=True)
    ...
    if len(str(tol_max)) > 0:
        ...
        for k in list_box:
            ...
            grass.run_command("v.db.update", map=output, column="OSM",
                              value=real_l_osm, where="cat={st}".format(st=k),
                              quiet=True)
                ...
                while exit == 0:
                    ...
                    if val >= l_osm: # all in
                        ...
                        if val >= l_osm:  # all in (considering epsilon)
                            ...
                            mid = new_mid
                if exit == 1:
                    grass.run_command("v.db.update", map=output, column="TOL",
                                      value=(math.ceil(x * 100)) / 100,
                                      where="cat={st}".format(st=k))
                grass.run_command("g.remove", type="vect", pattern=processid,
                                  flags="fr")
            grass.run_command("g.remove", type="vect", pattern=processid,
